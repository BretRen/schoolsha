...
export function updateElo(
  ...
  // 确保两条记录存在
  if (!data[winnerId]) {
    data[winnerId] = { elo: INITIAL_ELO, wins: 0, losses: 0, displayName: winnerName || winnerId };
  }
  if (!data[loserId]) {
    data[loserId] = { elo: INITIAL_ELO, wins: 0, losses: 0, displayName: loserName || loserId };
  }
  ...
  saveElo(data);
  console.log(
    `[elo] ${w.displayName} (${w.elo}) beat ${l.displayName} (${l.elo})`,
  );
}
