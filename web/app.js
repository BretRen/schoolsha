...
async function fetchDisconnectedGames() {
  ...
  try {
    const r = await fetch(
      `${HTTP_URL}/api/disconnected-games?token=${
    ...
    if (data.games?.length) {
      ...
      createOverlay(
        "🔌 断线重连",
        `房间 <b style="color:#c4b5fd">${g.roomCode}</b> &nbsp; 对手: ${
          esc(g.opponent)
        }`,
        remain,
        (t) => `${t} 秒内可重连`,
...
function createOverlay(
  ...
  const ignoreHtml = noIgnore
    ? ""
    : '<button class="btn btn-outline btn-sm" id="bl-ignore">忽略</button>';
  el.innerHTML =
    `<div style="background:#1a1a1a;border:2px solid #7c3aed;border-radius:16px;padding:40px;text-align:center;max-width:360px;display:flex;flex-direction:column;gap:12px">
    <h2 style="font-size:28px">${title}</h2><div>${body}</div>
    <p id="bl-countdown" style="color:#f59e0b;font-family:monospace;font-size:16px">${
      cfn(seconds)
    }</p>
    <div style="display:flex;gap:12px;justify-content:center;margin-top:8px">
      <button class="btn btn-primary btn-sm" id="bl-action">${
      onAction ? (buttonText || "重新连接") : ""
    }</button>
      ${ignoreHtml}</div></div>`;
...
function showToast(msg) {
  const el = document.createElement("div");
  el.style.cssText =
    "position:fixed;top:60px;left:50%;transform:translateX(-50%);background:rgba(220,38,38,.9);color:white;padding:8px 20px;border-radius:8px;font-size:14px;z-index:999;pointer-events:none;animation:fadeOut 3s forwards";
  el.textContent = msg;
  document.body.appendChild(el);
...
function showLoginOverlay() {
  ...
  el.id = "block-overlay";
  el.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;z-index:999";
  el.innerHTML =
    `<div style="background:var(--c-card);border:2px solid var(--c-accent);border-radius:16px;padding:40px 32px;text-align:center;max-width:340px;display:flex;flex-direction:column;gap:16px">
...
function createRoom() {
  ...
  fetch(`${HTTP_URL}/room/create`).then((r) => r.json()).then((info) => {
    const store = Alpine.store("g");
    store.roomCode = info.code;
    store.mode = "room";
    store.screen = "lobby";
    store.lobbyCode = info.code;
    store.lobbyInvite = info.inviteUrl;
...
function backToMenu() {
  ...
  store.selectedCards = {};
  store.blocked = false;
  store.matchStartTime = null;
  store.eloResult = null;
  store._lastLogLen = 0;
  store._lastPlayId = null;
  store._lastDiscardKeys = "";
  store.selectedChar = null;
  store.charLocked = false;
  store.opponentPicked = false;
  store.opponentLocked = false;
...
function showLeaderboard() {
  const store = Alpine.store("g");
  fetch(
    `${HTTP_URL}/leaderboard` +
      (store.gs?.playerId ? `?userId=${store.gs.playerId}` : ""),
  )
    .then((r) => r.json()).then((data) => {
      store.lbData = data;
      store.screen = "leaderboard";
    });
}
...
document.addEventListener("alpine:init", () => {
  Alpine.store("g", {
    ...
    toggleCard(id) {
      ...
      const isDiscard = this.gs?.phase === "discard" &&
        this.gs?.turnPlayer === this.myIndex;
      const isSkillDiscard = this.pending?.type === "skill_discard" &&
        this.isMyResp;
    ...
    isCardDisabled(c) {
      ...
      if (
        !isDiscard && this.gs?.phase === "play" && this.isMyTurn &&
        !this.pending && DEFENSIVE_ONLY.includes(c.name)
      ) return true;
      // 非响应阶段所有牌可用
      ...
      if (
        c.name === "免罚券" &&
        ["barbarian", "volley", "duel", "borrow_knife"].includes(p.type)
      ) return false;
      return true;
    },
    disabledReason(c) {
    ...
    isPickSelected(id) {
      return !!this._pickSelections[id];
    },
    doPickDiscard() {
    ...
    pendingLabel() {
      const p = this.pending;
      if (!p) return "";
      const label = this.isMyResp
        ? (RESP_NAMES[p.type] || p.type)
        : (RESP_NAMES_OPP[p.type] || p.type);
      ...
      switch (last.id) {
        ...
        case "judge_result":
          return `涂改液判定：${who} 翻出 ${
            ...
            last.result === "success" ? "红色·闪避成功 ✅" : "黑色·判定失败 ❌"
          }`;
        case "death":
          return `${who} 阵亡`;
        default:
    ...
    formatLogEntry(e) {
      ...
      switch (e.id) {
        case "card_played":
          return `${p} 使用了【${e.cardName}】${
            e.target !== undefined ? ` → P${e.target}` : ""
          }`;
        case "card_equipped":
        ...
        case "death":
          return `${p} 阵亡`;
        case "judge_result":
          return `⚖ 判定：${p} 翻出【${e.cardName}】${e.suit} → ${
            e.result === "success" ? "红色·闪避✅" : "黑色·生效❌"
          }`;
        default:
          return "";
      }
