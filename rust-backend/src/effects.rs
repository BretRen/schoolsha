//! effects.rs — 卡牌效果系统（游戏核心）
//!
//! 本模块是学校杀后端的核心，包含三大功能：
//!
//! ## 1. 效果注册表
//! 通过 `init_effects()` 注册全部 20+ 张卡牌的效果：
//! - **基本牌**: 作业(攻击)、豁免(闪避)、补给(回血)、小抄(酒+濒死)
//! - **效果牌**: 熬夜复习(残血翻盘)、请家长(概率控场)
//! - **锦囊牌**: 辩论(轮流出作业)、突击测验(出作业或扣血)、最终测试(双方摸牌)、嫁祸(弃牌或扣血)
//! - **即时牌**: 神偷(偷牌)、告密(弃牌)、陷害(盲选弃2)、点名批评(需豁免)、午饭留堂(盲选弃1)、午饭(摸2)、感冒(直接1伤)
//! - **装备牌**: 钢笔(+1伤)、圆规(连击)、尺子(豁免后重置攻击)、橡皮(豁免后仍受伤)、校服(防黑作业)、黑名单(免疫作业)、涂改液(判定)
//! - **响应牌**: 免罚券(取消锦囊)
//!
//! ## 2. 核心流程
//! - `try_use_card()` — 出牌入口，自动路由到 `try_respond()` 或效果执行
//! - `try_respond()` — 处理 pending 响应（豁免/补给/小抄/作业/任意牌/免罚券）
//! - `handle_timeout()` — 超时处理（扣血/死亡/随机偷牌/取消技能）
//!
//! ## 3. 特殊机制
//! - `deal_damage()` — 伤害计算（黑名单加成、触发濒死）
//! - `handle_steal_card()` — 盲选偷牌（装备公开、手牌盲选）
//! - `handle_activate_armor()` — 涂改液主动判定
//! - `handle_pick_discard()` — 陷害/午饭留堂盲选弃牌
//! - `handle_confirm_skill()` — 技能代价弃牌确认后执行
